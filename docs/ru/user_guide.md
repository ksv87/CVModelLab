...
Кнопка темы в панели приложения переключает режим между **Системная**, **Светлая** и **Тёмная**. Текущий режим показан иконкой кнопки, активный вариант отмечен галочкой в меню. Выбор сохраняется и восстанавливается при следующем запуске; **Системная** следует оформлению операционной системы и меняется вживую при его изменении, без перезапуска. Переключатель находится рядом с выбором языка на стартовом экране и в workspace.
